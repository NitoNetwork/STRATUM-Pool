...
static bool server_alive(nitopool_t *ckp, server_instance_t *si, bool pinging)
{
	char *userpass = NULL;
	bool ret = false;
	connsock_t *cs;
	...
	clear_gbtbase(&gbt);
	if (unlikely(ckp->nitosolo && !ckp->nitoaddress)) {
		/* If no nitoaddress is specified in solonito mode, choose one of
		...
		if (validate_address(cs, ckp->donaddress, &ckp->script, &ckp->segwit))
			ckp->nitoaddress = ckp->donaddress;
		else if (validate_address(cs, ckp->tndonaddress, &ckp->script, &ckp->segwit))
			ckp->nitoaddress = ckp->tndonaddress;
		else if (validate_address(cs, ckp->rtdonaddress, &ckp->script, &ckp->segwit))
			ckp->nitoaddress = ckp->rtdonaddress;
	}

	if (!ckp->node && !validate_address(cs, ckp->nitoaddress, &ckp->script, &ckp->segwit)) {
		LOGWARNING("Invalid nitoaddress: %s !", ckp->nitoaddress);
		goto out;
	}
...
static server_instance_t *live_server(nitopool_t *ckp, gdata_t *gdata)
{
	server_instance_t *alive = NULL;
	connsock_t *cs;
	int i;

	LOGDEBUG("Attempting to connect to nitod");
retry:
	...
	LOGWARNING("CRITICAL: No nitods active!");
	sleep(5);
	goto retry;
living:
...
reconnect:
	...
	cs = &si->cs;
	if (!old_si)
		LOGWARNING("Connected to nitod: %s:%s", cs->url, cs->port);
	else if (si != old_si)
		LOGWARNING("Failed over to nitod: %s:%s", cs->url, cs->port);
...
retry:
	...
	if (unlikely(!si->alive)) {
		LOGWARNING("%s:%s Nitod socket invalidated, will attempt failover", cs->url, cs->port);
		goto reconnect;
	}
...
static bool connect_proxy(nitopool_t *ckp, connsock_t *cs, proxy_instance_t *proxy)
{
	if (cs->fd > 0) {
...
static bool subscribe_stratum(nitopool_t *ckp, connsock_t *cs, proxy_instance_t *proxi)
{
	bool ret = false;
	json_t *req;
...
static void send_notify(nitopool_t *ckp, proxy_instance_t *proxi, notify_instance_t *ni);

static void reconnect_generator(nitopool_t *ckp)
{
...
static bool parse_notify(nitopool_t *ckp, proxy_instance_t *proxi, json_t *val)
{
...
static proxy_instance_t *create_subproxy(nitopool_t *ckp, gdata_t *gdata, proxy_instance_t *proxi,
...
static void send_stratifier_deadproxy(nitopool_t *ckp, const int id, const int subid)
{
	char buf[256];
...
static void send_stratifier_delproxy(nitopool_t *ckp, const int id, const int subid)
{
	char buf[256];
...
static void send_diff(nitopool_t *ckp, proxy_instance_t *proxi)
{
...
static bool parse_method(nitopool_t *ckp, proxy_instance_t *proxi, const char *msg)
{
...
/* cs semaphore must be held */
static bool auth_stratum(nitopool_t *ckp, connsock_t *cs, proxy_instance_t *proxi)
{
...
static void send_subscribe(nitopool_t *ckp, proxy_instance_t *proxi)
{
	json_t *json_msg;
	char *msg, *buf;
...
static void stratifier_reconnect_client(nitopool_t *ckp, const int64_t id)
{
	char buf[256];
...
static void passthrough_send(nitopool_t *ckp, pass_msg_t *pm)
{
...
static void suggest_diff(nitopool_t *ckp, connsock_t *cs, proxy_instance_t *proxy)
{
	json_t *req;
	bool ret;
...
static bool proxy_alive(nitopool_t *ckp, proxy_instance_t *proxi, connsock_t *cs,
			bool pinging)
{
...
static proxy_instance_t *wait_best_proxy(nitopool_t *ckp, gdata_t *gdata)
{
...
static proxy_instance_t *__add_proxy(nitopool_t *ckp, gdata_t *gdata, const int num);

static proxy_instance_t *__add_userproxy(nitopool_t *ckp, gdata_t *gdata, const int id,
...
static void add_userproxy(nitopool_t *ckp, gdata_t *gdata, const int userid,
...
static void parse_addproxy(nitopool_t *ckp, gdata_t *gdata, const int sockd, const char *buf)
{
...
static void delete_proxy(nitopool_t *ckp, gdata_t *gdata, proxy_instance_t *proxy)
{
	proxy_instance_t *subproxy;
...
static void parse_delproxy(nitopool_t *ckp, gdata_t *gdata, const int sockd, const char *buf)
{
	json_t *val = NULL, *res = NULL;
	proxy_instance_t *proxy;
	json_error_t err_val;
...
static void parse_globaluser(nitopool_t *ckp, gdata_t *gdata, const char *buf)
{
...
static void setup_servers(nitopool_t *ckp)
{
	pthread_t pth_watchdog;
	int i;

	ckp->servers = ckalloc(sizeof(server_instance_t *) * ckp->nitods);
	for (i = 0; i < ckp->nitods; i++) {
		server_instance_t *si;
		connsock_t *cs;
		...
		si = ckp->servers[i];
		si->url = ckp->nitodurl[i];
		si->auth = ckp->nitodauth[i];
		si->pass = ckp->nitodpass[i];
		si->notify = ckp->nitodnotify[i];
		si->id = i;
		cs = &si->cs;
		cs->ckp = ckp;
		cksem_init(&cs->sem);
...
static void server_mode(nitopool_t *ckp, proc_instance_t *pi)
{
	int i;
...
static proxy_instance_t *__add_proxy(nitopool_t *ckp, gdata_t *gdata, const int id)
{
	proxy_instance_t *proxy;
...
static void proxy_mode(nitopool_t *ckp, proc_instance_t *pi)
{
	gdata_t *gdata = ckp->gdata;
	proxy_instance_t *proxy;
	int i;
