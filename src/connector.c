...
void connector_upstream_msg(nitopool_t *ckp, char *msg)
{
...
int64_t connector_newclientid(nitopool_t *ckp)
{
	int64_t ret;
...
static void stratifier_drop_id(nitopool_t *ckp, const int64_t id)
{
	char buf[256];
...
static void generator_drop_client(nitopool_t *ckp, const client_instance_t *client)
{
	json_t *val;
...
static void stratifier_drop_client(nitopool_t *ckp, const client_instance_t *client)
{
 ...
 * count. */
static int invalidate_client(nitopool_t *ckp, cdata_t *cdata, client_instance_t *client)
{
...
static void send_client(nitopool_t *ckp, cdata_t *cdata, int64_t id, char *buf);
...
static bool parse_client_msg(nitopool_t *ckp, cdata_t *cdata, client_instance_t *client)
{
	int buflen, ret;
	json_t *val;
	char *eol;
...
static void redirect_client(nitopool_t *ckp, client_instance_t *client);
...
static void client_event_processor(nitopool_t *ckp, struct epoll_event *event)
{
...
static bool send_sender_send(nitopool_t *ckp, cdata_t *cdata, sender_send_t *sender_send)
{
...
static int add_redirect(nitopool_t *ckp, cdata_t *cdata, client_instance_t *client)
{
	redirect_t *redirect;
	bool found;
 ...
 * free the ram. */
static void send_client(nitopool_t *ckp, cdata_t *cdata, const int64_t id, char *buf)
{
	sender_send_t *sender_send;
	client_instance_t *client;
	bool redirect = false;
...
static void send_client_json(nitopool_t *ckp, cdata_t *cdata, int64_t client_id, json_t *json_msg)
{
	client_instance_t *client;
	char *msg;
...
static void passthrough_client(nitopool_t *ckp, cdata_t *cdata, client_instance_t *client)
{
	json_t *val;
...
static bool connect_upstream(nitopool_t *ckp, connsock_t *cs)
{
...
static void usend_process(nitopool_t *ckp, char *buf)
{
	cdata_t *cdata = ckp->cdata;
...
static bool setup_upstream(nitopool_t *ckp, cdata_t *cdata)
{
	connsock_t *cs = &cdata->upstream_cs;
	bool ret = false;
	pthread_t pth;
...
static void client_message_processor(nitopool_t *ckp, json_t *json_msg)
{
	cdata_t *cdata = ckp->cdata;
	client_instance_t *client;
	int64_t client_id;
...
void connector_add_message(nitopool_t *ckp, json_t *val)
{
	cdata_t *cdata = ckp->cdata;
...
static void drop_passthrough_client(nitopool_t *ckp, cdata_t *cdata, const int64_t id)
{
	int64_t client_id;
	char *msg;
...
void connector_send_fd(nitopool_t *ckp, const int fdno, const int sockd)
{
	cdata_t *cdata = ckp->cdata;
