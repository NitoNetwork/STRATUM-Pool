...
struct nitopool_instance;
typedef struct nitopool_instance nitopool_t;
...
struct nitopool_instance {
	...
	/* Are we running in nitosolo mode */
	bool nitosolo;

	/* Are we running in userproxy mode */
	bool userproxy;

	/* Should we daemonise the nitopool process */
	bool daemon;
	...
	char **nitodpass;
	bool *nitodnotify;
	int blockpoll; // How frequently in ms to poll nitod for block updates
	...
	/* Coinbase data */
	char *nitoaddress; // Address to mine to
	bool script; // Address is a script address
	bool segwit; // Address is a segwit address
	char *nitosig; // Optional signature to add to coinbase
...
char *_send_recv_ckdb(const nitopool_t *ckp, const char *msg, const char *file, const char *func, const int line);
#define send_recv_ckdb(ckp, msg) _send_recv_ckdb(ckp, msg, __FILE__, __func__, __LINE__)
char *_ckdb_msg_call(const nitopool_t *ckp, const char *msg,  const char *file, const char *func,
		     const int line);
...
static inline void nitopool_api(nitopool_t __maybe_unused *ckp, apimsg_t __maybe_unused *apimsg) {};
