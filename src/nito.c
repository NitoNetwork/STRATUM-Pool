...
/* Take a nito address and do some sanity checks on it, then send it to
 * nitod to see if it's a valid address */
bool validate_address(connsock_t *cs, const char *address, bool *script, bool *segwit)
{
	...
	if (!json_is_true(valid_val)) {
		LOGDEBUG("Nito address %s is NOT valid", address);
		goto out;
	}
	ret = true;
	tmp_val = json_object_get(res_val, "isscript");
	if (unlikely(!tmp_val)) {
		/* All recent nitods with wallet support built in should
		 * support this, if not, look for addresses the braindead way
		 * to tell if it's a script address. */
		LOGDEBUG("No isscript support from nitod");
	...
	*segwit = json_is_true(tmp_val);
	LOGDEBUG("Nito address %s IS valid%s%s", address, *script ? " script" : "",
		 *segwit ? " segwit" : "");
out:
	if (val)
		json_decref(val);
...
/* Request getblocktemplate from nitod already connected with a connsock_t
...
/* Request getblockcount from nitod, returning the count or -1 if the call
 * fails. */
...
/* Request getblockhash from nitod for height, writing the value into *hash
...
/* Request getbestblockhash from nitod. nitod 0.9+ only */
