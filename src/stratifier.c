...
static void generate_coinbase(nitopool_t *ckp, workbase_t *wb)
{
	...
	wb->coinb2len = 7;
	if (ckp->nitosig) {
		int siglen = strlen(ckp->nitosig);

		LOGDEBUG("Len %d sig %s", siglen, ckp->nitosig);
		if (siglen) {
			wb->coinb2bin[wb->coinb2len++] = siglen;
			memcpy(wb->coinb2bin + wb->coinb2len, ckp->nitosig, siglen);
			wb->coinb2len += siglen;
		}
	}
	len += wb->coinb2len;
	...
	*u64 = htole64(g64);
	...
	if (!ckp->nitosolo) {
		...
		if (unlikely(!ckp->coinbase_valid)) {
			...
			LOGWARNING("Mining from any incoming username to address %s", ckp->nitoaddress);
			if (ckp->donation)
		...
		free(cb);
		ckp->coinbase_valid = true;
		LOGWARNING("Mining solo to any incoming valid NITO address username");
		if (ckp->donation)
			LOGWARNING("%.1f percent donation to %s", ckp->donation, ckp->donaddress);
	}
...
static void clear_workbase(nitopool_t *ckp, workbase_t *wb)
{
	if (ckp->nitosolo)
...
static void upstream_json(nitopool_t *ckp, json_t *val)
{
	char *msg;
...
static void upstream_json_msgtype(nitopool_t *ckp, json_t *val, const int msg_type)
{
...
static void upstream_msgtype(nitopool_t *ckp, const json_t *val, const int msg_type)
{
...
static void send_node_workinfo(nitopool_t *ckp, sdata_t *sdata, const workbase_t *wb)
{
...
static json_t *generate_workinfo(nitopool_t *ckp, const workbase_t *wb, const char *func)
{
	char cdfield[64];
	json_t *val;
...
static void send_workinfo(nitopool_t *ckp, sdata_t *sdata, const workbase_t *wb)
{
	if (!ckp->proxy)
...
static void add_base(nitopool_t *ckp, sdata_t *sdata, workbase_t *wb, bool *new_block)
{
...
/* Submit the transactions in node/remote mode so the local nitod has all the
 * transactions that will go into the next blocksolve. */
static void submit_transaction(nitopool_t *ckp, const char *hash)
{
	char *buf;
...
static bool add_txn(nitopool_t *ckp, sdata_t *sdata, txntable_t **txns, const char *hash,
		    const char *data, bool local)
{
	...
	if (local)
		...
		/* Get the data from our local nitod as a way of confirming it
		 * already knows about this transaction. */
		txn->data = generator_get_txn(ckp, hash);
		if (!txn->data) {
			/* If our local nitod hasn't seen this transaction,
...
static void send_node_transactions(nitopool_t *ckp, sdata_t *sdata, const json_t *txn_val)
{
	stratum_instance_t *client;
	ckmsg_t *bulk_send = NULL;
	ckmsg_t *client_msg;
...
static void submit_transaction_array(nitopool_t *ckp, const json_t *arr)
{
	json_t *arr_val;
	size_t index;
...
static void update_txns(nitopool_t *ckp, sdata_t *sdata, txntable_t *txns, bool local)
{
	...
	/* Submit transactions to nitod again when we're purging them in
...
static txntable_t *wb_merkle_bin_txns(nitopool_t *ckp, sdata_t *sdata, workbase_t *wb,
 ...
 * are serialised. */
static void block_update(nitopool_t *ckp, int *prio)
{
...
retry:
	...
	if (likely(witnessdata_check)) {
		...
		if (wb->insert_witness && safecmp(witnessdata_check + 4, wb->witnessdata) != 0)
			LOGERR("Witness from nitod: %s. Calculated Witness: %s", witnessdata_check + 4, wb->witnessdata);
	}
...
out:
	 ...
	 * connected while nitod recovers(?) */
	if (unlikely(!ret)) {
...
static void request_txns(nitopool_t *ckp, sdata_t *sdata, json_t *txns)
{
	json_t *val;
 ...
 * on remote workbases */
static bool rebuild_txns(nitopool_t *ckp, sdata_t *sdata, workbase_t *wb)
{
	...
	for (i = 0; i < wb->txns; i++) {
		...
		if (likely(txn_val))
			continue;
		/* See if we can find it in our local nitod */
...
static void add_remote_base(nitopool_t *ckp, sdata_t *sdata, workbase_t *wb)
{
	stratum_instance_t *client;
	ckmsg_t *bulk_send = NULL;
	workbase_t *tmp, *tmpa;
...
static void add_node_base(nitopool_t *ckp, json_t *val, bool trusted, int64_t client_id)
{
...
static void add_remote_blockdata(nitopool_t *ckp, json_t *val, const int cblen, const char *coinbase,
				 const uchar *data)
{
	char *buf;
...
static void send_node_block(nitopool_t *ckp, sdata_t *sdata, const char *enonce1, const char *nonce,
...
static bool local_block_submit(nitopool_t *ckp, char *gbt_block, const uchar *flip32, int height)
{
...
static void block_solve(nitopool_t *ckp, json_t *val);
static void block_reject(json_t *val);

static void submit_node_block(nitopool_t *ckp, sdata_t *sdata, json_t *val)
{
...
static void connector_drop_client(nitopool_t *ckp, const int64_t id)
{
	char buf[256];
...
static void drop_allclients(nitopool_t *ckp)
{
...
static void generator_recruit(nitopool_t *ckp, const int proxyid, const int recruits)
{
	char buf[256];
...
static void update_subscribe(nitopool_t *ckp, const char *cmd)
{
...
static void update_notify(nitopool_t *ckp, const char *cmd)
{
	sdata_t *sdata = ckp->sdata, *dsdata;
...
static void update_diff(nitopool_t *ckp, const char *cmd)
{
	sdata_t *sdata = ckp->sdata, *dsdata;
	stratum_instance_t *client, *tmp;
...
#if 0
static void generator_drop_proxy(nitopool_t *ckp, const int64_t id, const int subid)
{
	char msg[256];
...
static void free_proxy(nitopool_t *ckp, proxy_t *proxy)
{
 ...
 * those. */
static void reap_proxies(nitopool_t *ckp, sdata_t *sdata)
{
...
static stratum_instance_t *__stratum_add_instance(nitopool_t *ckp, int64_t id, const char *address,
						  int server)
{
	sdata_t *sdata = ckp->sdata;
	stratum_instance_t *client;
	...
	client->ckp = ckp;
	tv_time(&client->ldc);
	/* Points to ckp sdata in nitopool mode, but is changed later in proxy
	 * mode . */
	client->sdata = sdata;
...
static void connector_test_client(nitopool_t *ckp, const int64_t id)
{
	char buf[256];
 ...
 * locks. Sends only to sdata bound clients (everyone in nitopool) */
static void stratum_broadcast(sdata_t *sdata, json_t *val, const int msg_type)
{
	nitopool_t *ckp = sdata->ckp;
	sdata_t *ckp_sdata = ckp->sdata;
	stratum_instance_t *client, *tmp;
	ckmsg_t *bulk_send = NULL;
	int messages = 0;
...
static void drop_client(nitopool_t *ckp, sdata_t *sdata, const int64_t id)
{
	char_entry_t *entries = NULL;
	stratum_instance_t *client;
	char *msg = NULL;
...
char *stratifier_stats(nitopool_t *ckp, void *data)
{
...
static void dead_proxy(nitopool_t *ckp, sdata_t *sdata, const char *buf)
{
	int id = 0, subid = 0;
...
static void del_proxy(nitopool_t *ckp, sdata_t *sdata, const char *buf)
{
	int id = 0, subid = 0;
...
static void stratum_loop(nitopool_t *ckp, proc_instance_t *pi)
{
	sdata_t *sdata = ckp->sdata;
...
static bool new_enonce1(nitopool_t *ckp, sdata_t *ckp_sdata, sdata_t *sdata, stratum_instance_t *client)
{
	proxy_t *proxy = NULL;
	uint64_t enonce1;
 ...
 * running out of room. */
static sdata_t *select_sdata(nitopool_t *ckp, sdata_t *ckp_sdata, const int userid)
{
...
static void read_userstats(nitopool_t *ckp, sdata_t *sdata, int tvsec_diff)
{
 ...
 * count. */
static user_instance_t *generate_user(nitopool_t *ckp, stratum_instance_t *client,
				      const char *workername)
{
	...
	if (!ckp->proxy && (new_user || !user->nitoaddress)) {
		/* Is this a nito address based username? */
		if (generator_checkaddr(ckp, username, &user->script, &user->segwit)) {
			user->nitoaddress = true;
			user->txnlen = address_to_txn(user->txnbin, username, user->script, user->segwit);
		}
	}
	if (new_user) {
		LOGNOTICE("Added new user %s%s", username, user->nitoaddress ?
...
static void check_global_user(nitopool_t *ckp, user_instance_t *user, stratum_instance_t *client)
{
	sdata_t *sdata = ckp->sdata;
...
static void client_auth(nitopool_t *ckp, stratum_instance_t *client, user_instance_t *user,
			const bool ret)
{
	if (ret) {
...
/* Needs to be entered with client holding a ref count. */
static void add_submit(nitopool_t *ckp, stratum_instance_t *client, const double diff, const bool valid,
		       const bool submit)
{
...
static void
downstream_block(nitopool_t *ckp, sdata_t *sdata, const json_t *val, const int cblen,
 ...
 * client holding a ref count. */
static void suggest_diff(nitopool_t *ckp, stratum_instance_t *client, const char *method,
 ...
 * finished. */
static void add_mining_node(nitopool_t *ckp, sdata_t *sdata, stratum_instance_t *client)
{
	pthread_t pth;
...
static void parse_method(nitopool_t *ckp, sdata_t *sdata, stratum_instance_t *client,
...
static void parse_share_result(nitopool_t *ckp, stratum_instance_t *client, json_t *val)
{
	time_t now_t;
	ts_t now;
...
static void parse_authorise_result(nitopool_t *ckp, sdata_t *sdata, stratum_instance_t *client,
				   json_t *val)
{
	if (!json_is_true(val)) {
...
static user_instance_t *generate_remote_user(nitopool_t *ckp, const char *workername)
{
	char *base_username = strdupa(workername), *username;
	sdata_t *sdata = ckp->sdata;
	bool new_user = false;
	...
	if (new_user) {
		LOGNOTICE("Added new remote user %s%s", username, user->nitoaddress ?
			  " as address based registration" : "");
	}
...
static void parse_remote_share(nitopool_t *ckp, sdata_t *sdata, json_t *val, const char *buf)
{
...
static void parse_remote_shareerr(nitopool_t *ckp, json_t *val, const char *buf)
{
	const char *workername;
...
static void send_auth_success(nitopool_t *ckp, sdata_t *sdata, stratum_instance_t *client)
{
	char *buf;
...
void parse_upstream_auth(nitopool_t *ckp, json_t *val)
{
...
void parse_upstream_workinfo(nitopool_t *ckp, json_t *val)
{
...
static void parse_remote_auth(nitopool_t *ckp, sdata_t *sdata, json_t *val, stratum_instance_t *remote,
			      const int64_t remote_id)
{
	...
	if (ckp->nitosolo) {
		LOGWARNING("Got remote auth request in nitosolo mode, ignoring!");
		return;
	}
...
static void parse_remote_block(nitopool_t *ckp, sdata_t *sdata, json_t *val, const char *buf,
...
void parse_upstream_block(nitopool_t *ckp, json_t *val)
{
	char *buf;
	sdata_t *sdata = ckp->sdata;
...
static void add_node_txns(nitopool_t *ckp, sdata_t *sdata, const json_t *val)
{
...
void parse_remote_txns(nitopool_t *ckp, const json_t *val)
{
...
void parse_upstream_reqtxns(nitopool_t *ckp, json_t *val)
{
...
static void parse_trusted_msg(nitopool_t *ckp, sdata_t *sdata, json_t *val, stratum_instance_t *client)
{
...
static void node_client_msg(nitopool_t *ckp, json_t *val, stratum_instance_t *client)
{
...
static void parse_node_msg(nitopool_t *ckp, sdata_t *sdata, json_t *val)
{
	int msg_type = node_msg_type(val);
...
/* Entered with client holding ref count */
static void parse_instance_msg(nitopool_t *ckp, sdata_t *sdata, smsg_t *msg, stratum_instance_t *client)
{
	...
	while (unlikely(!ckp->proxy && !sdata->current_workbase)) {
		cksleep_ms(100);
		if (!(++delays % 50))
			LOGWARNING("%d Second delay waiting for nitod at startup", delays / 10);
	}
	parse_method(ckp, sdata, client, client_id, id_val, method, params);
}

static void srecv_process(nitopool_t *ckp, json_t *val)
{
...
void _stratifier_add_recv(nitopool_t *ckp, json_t *val, const char *file, const char *func, const int line)
{
	sdata_t *sdata;
...
static void ssend_process(nitopool_t *ckp, smsg_t *msg)
{
...
static void sshare_process(nitopool_t *ckp, json_params_t *jp)
{
...
static void upstream_auth(nitopool_t *ckp, stratum_instance_t *client, json_params_t *jp)
{
	json_t *val = json_object();
	char cdfield[64];
	char *msg;
...
static void sauth_process(nitopool_t *ckp, json_params_t *jp)
{
...
static void send_transactions(nitopool_t *ckp, json_params_t *jp)
{
...
static void upstream_workers(nitopool_t *ckp, user_instance_t *user)
{
	char *msg;
...
static void read_poolstats(nitopool_t *ckp, int *tvsec_diff)
{
...
void *stratifier(void *arg)
{
	...
	if (!ckp->proxy) {
		if (!generator_checkaddr(ckp, ckp->nitoaddress, &ckp->script, &ckp->segwit)) {
			LOGEMERG("Fatal: nitoaddress invalid according to nitod");
			goto out;
		}

		/* Store this for use elsewhere */
		hex2bin(scriptsig_header_bin, scriptsig_header, 41);
		sdata->txnlen = address_to_txn(sdata->txnbin, ckp->nitoaddress, ckp->script, ckp->segwit);
